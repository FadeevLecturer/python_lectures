...
```{note}
Если во время установке вы не добавили `python` в переменную среды `PATH`, то чтобы вызвать интерактивную оболочку из командной стоки придется сначала перейти в папку с установленным `python`.
```
...
```
```{note}
В большинстве случаев функции возвращают какое-то значение, которое может быть непосредственно задействовано в дальнейших вычислениях или связано с переменной оператором присваивания "`=`".
```
